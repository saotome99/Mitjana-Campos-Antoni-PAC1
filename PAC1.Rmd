...
# Filtra les dades per mantenir només els pacients comuns
common_patients <- intersect(colnames(metabolite_data), rownames(metadata))
metabolite_data <- metabolite_data[, common_patients, drop = FALSE]
metadata <- metadata[common_patients, , drop = FALSE]

# Verifica les dimensions després de la transposició
cat("Dimensions de metabolite_data (transposat):", dim(metabolite_data), "\n")
cat("Dimensions de metadata:", dim(metadata), "\n")
